...
| [PTP-adjacent records and dispatch](firmware/PTP.md) | Registration callers, FIFO mechanics, selectors, callbacks, descriptors, queued storage, a [qualified USB-state reporting wrapper](firmware/PTP.md#usb-state-wrapper), [conditional connection-candidate byte stores and consumer](firmware/PTP.md#usb-connect-stores), and a [named MTP lifecycle with range-only aggregate/selector and receive-record/consumer candidates](firmware/PTP.md#mtp-communication-lifecycle) |
